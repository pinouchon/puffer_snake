...
typedef struct CSnakeV1 CSnakeV1;
struct CSnakeV1 {
    char* observations;
    int* actions;
    float* rewards;
...
static inline unsigned int rng_next(CSnakeV1* env) {
...
static inline int rng_bounded(CSnakeV1* env, int bound) {
    if (bound <= 1) {
        return 0;
    }
...
static inline int pos_to_r(CSnakeV1* env, int pos) {
    return pos / env->width;
}

static inline int pos_to_c(CSnakeV1* env, int pos) {
    return pos % env->width;
}

static inline int rc_to_pos(CSnakeV1* env, int r, int c) {
...
static inline void add_log(CSnakeV1* env) {
...
static inline int ring_prev(CSnakeV1* env, int ptr) {
    return (ptr - 1 + env->area) % env->area;
}

static inline int ring_tail(CSnakeV1* env) {
    return (env->head_ptr - env->snake_length + 1 + env->area) % env->area;
}

static inline void configure(CSnakeV1* env) {
...
void c_close(CSnakeV1* env) {
    free(env->grid);
    free(env->snake);
}

static inline void clear_snake(CSnakeV1* env) {
...
static inline bool spawn_food(CSnakeV1* env) {
    int empty_count = 0;
...
static inline void ensure_food(CSnakeV1* env) {
...
static inline void spawn_snake(CSnakeV1* env) {
    clear_snake(env);
...
static inline void compute_observations(CSnakeV1* env) {
    for (int pos = 0; pos < env->area; pos++) {
...
static inline void end_episode(CSnakeV1* env, float terminal_reward) {
    env->terminals[0] = 1;
