...
static int my_init(Env* env, PyObject* args, PyObject* kwargs) {
    ...
    env->num_food = unpack(kwargs, "num_food");
    ...
    env->shape_on_eat = unpack(kwargs, "shape_on_eat");
...
static char tile_to_ascii(unsigned char tile) {
    switch (tile) {
        case EMPTY: return ' ';
        case FOOD: return '*';
...
static PyObject* vec_render_ansi(PyObject* self, PyObject* args) {
    ...
    for (int y = 0; y < env->height; y += stride) {
        for (int x = 0; x < env->width; x += stride) {
            unsigned char tile = env->grid[y * env->width + x];
